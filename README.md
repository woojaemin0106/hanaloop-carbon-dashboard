...
과제 제공 활동 데이터를 기준으로 제품 `CT-045`의 총 배출량, 제품 단위 PCF, GHG Scope별 비중, 월별 추이를 한 화면에서 확인하고 주요 활동량을 조정해 감축 시나리오를 검토할 수 있습니다.
...
- 입력 UX: 생산수량 조정, 상위 배출 활동량 조정, 단위 PCF/총 배출량 즉시 재계산, 오류 메시지 표시
- 내부 API: `/api/pcf/summary`
- 테스트: PCF 계산 테스트, 생산수량/활동량 입력 검증 테스트, API 응답 테스트, 앱 진입점 smoke 테스트
...
| 제품 단위 PCF | `11,072.724 kgCO2e/과제 데이터 묶음` |
